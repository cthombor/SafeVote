stv <- function(votes, nseats = NULL, eps = 0.001, equal.ranking = FALSE,
    ...
    ##
    ## The argument votes (matrix or data frame) contains the votes
    ## themselves.  Row i of the matrix contains the preferences of
    ## voter i numbered 1, 2, .., r, 0,0,0,0, in some order The
    ## columns of the matrix correspond to the candidates.  The
    ## dimnames of the columns are the names of the candidates; if
    ## these are not supplied then the candidates are lettered A, B,
    ## C, ...
    ##
    ## If votes is a character string it is interpreted as a file name
    ## from which the votes are to be read. A tab delimited file
    ## produced by excel will be in the right format, with the
    ## candidate names in the first row.
    ##
    ## The argument nseats is the number of candidates to be elected.
    ##
    ## If nseats is not supplied and !complete.ranking, it will be
    ## assumed that the number of candidates to be elected is half the
    ## number of candidates standing.  However when completely ranking
    ## the candidates, it's arguably most appropriate to set nseats
    ## equal to the number of candidates -- so this is the default
    ## when complete.ranking = TRUE.
    ##
    ...
    ##
    ## Margins and SafeRanks were computed, and the code was
    ## reformatted for legibility, in November 2022.  SafeRank is the
    ## result of running a simple clustering heuristic to ensure a
    ## minimum margin (greater than the "fuzz") between the
    ## lowest-ranked candidate of each cluster and the highest-ranked
    ## candidate of every other cluster.  If voter preferences are
    ## i.u.d. then safety=1.0 is a "fuzz" of 1 s.d. on the
    ## vote-counts.
    ##
    ##################################
    ...
    if( complete.ranking ){
        ...
        nseats <- check.nseats(nseats, nc+1, default=nc, ...)
    } else {
    ...
    x <-
        check.votes(corvotes, "stv", equal.ranking = equal.ranking,
                    quiet = quiet)
    corrected <-
	...
	if(use.marking){
	    ...
	    if(use.marking && length(ic) > 1 && sum(Dm) <= group.nseats) {
            ...
            if(length(ic) > 1) {
                ## tie
                ic <-
                    solve.tiebreak(tie.method, result.pref, ic, otb,
                                   elim = FALSE)
                tie <- 1
            ...
            index <- (x[, ic] == 1) ## ballots where ic has the first
                                    ## preference
            ...
            elim.select <- D
	    if(use.marking &&
               (nseats == group.nseats || sum(Dm) <= group.nseats)) {
            ...
            if(verbose && !quiet) {
                cat("Candidate", cnames[ic],
                    "eliminated with margin", result.margins[ic]
                    )
                if(tie > 0) {
                    cat("using", tie.method.name[tie.method])
                    if(tie == 2) cat(" & ordered")
...
solve.tiebreak <- function(method, prefs, icans,
...
print.summary.vote.stv <- function(x, ...) {
    ...
    if(! is.null(attr(x, "rankingTable"))){
        ...
        cat("\nComplete ranking:")
        print(kable(attr(x, "rankingTable"), align = c("r", "l", "c", "r", "l"), ...))
    }
...
image.vote.stv <- function(x, xpref = 2, ypref = 1,
    ...
    if(all.pref) {
        ...
        image.plot(x = 1:nc, y = 1:nc, t(nij),
        ...
        image.plot(x = 1:nc, y = 1:nc, t(ctbl[nc:1,]),
                   axes = FALSE, xlab = "", ylab = "",
                   col = hcl.colors(12, "YlOrRd", rev = TRUE), ...)
...
plot.vote.stv <- function(x, xlab = "Count", ylab = "Preferences",
                          point.size = 2, ...) {
    ...
    dfe <- melt(data.table(Count = 1:nrow(x$elect.elim), x$elect.elim),
                id.vars = "Count",
...
#    names(safeRank) <- result$Candidate
