...
❯ checking CRAN incoming feasibility … \[13s\] NOTE Maintainer: ‘Clark
Thomborson <c.thomborson@auckland.ac.nz>’

New submission

❯ checking HTML version of manual … \[11s\] NOTE Skipping checking math
...
- The math-rendering NOTE seems likely to be the issue I had reported in
  ...
  It’s certainly the case that $\sqrt{x}$ is still not rendering
  properly in <https://cthombor.github.io/SafeVote/>, despite my many
  attempts to find a workaround. I’m unable to find the
  lastMiKTeXException file at rhub, so will try again in a few days, and
  if V8 is still unavailable I’ll file an issue report.
